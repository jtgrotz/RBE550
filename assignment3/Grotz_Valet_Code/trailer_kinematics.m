    ...
    methods
        ...
        function coll = check_collision(obj,costmap,points)
            ...
            if (coll == 0)
                ...
                trailer_points_y = points(:,2)-obj.hitch_length*sin(points(:,4));
                trailer_points = [trailer_points_x,trailer_points_y,points(:,4)];
                x_oob = any(trailer_points(:,1) >= (limits(2))) || any(trailer_points(:,1) <= (limits(1)));
                y_oob = any(trailer_points(:,2) >= (limits(4))) || any(trailer_points(:,2) <= (limits(3)));
                if (x_oob || y_oob)
                    coll = 0;
                else 
                    try
                        coll = any(checkOccupied(costmap,trailer_points(1:2)));
                    catch
                        coll = 0;
                    end
                end
